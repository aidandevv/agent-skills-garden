...
3. **Hooks** (`<skill>/hooks/*.sh`) — deterministic shell scripts wired into
   ...
   `PostCompact`, Codex `Stop`/`TaskCompleted`). Hooks exist because model-driven
   journaling is probabilistic and degrades near context limits; hooks fire
   regardless of model attention.
